...
export class BooksService {
  ...
  async create(createBookDto: CreateBookDto): Promise<any> {
    const result = await this.connection
      .query(
        'INSERT INTO book (buyinfo, title, author, description,publisher, cover) VALUES ($1, $2, $3, $4, $5, $6)',
        [
  ...
  async update(id: number, updateBookDto: UpdateBookDto): Promise<any> {
    const result = await this.connection
      .query(
        `UPDATE book SET buyinfo = $1, title = $2, author = $3, description = $4, publisher = $5, cover = $6 WHERE id = $7`,
        [
